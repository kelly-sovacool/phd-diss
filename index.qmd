...
such as waterways, soil layers, ocean floors, and body parts of plants, insects,
and animals.
The human body hosts microbes that inhabit the skin, mouth, gut, vagina, airway,
and other body regions [@hmp_structure_2012].
...
techniques are required to process and analyze big data [@hmp_structure_2012].
...
region are amplified and sequenced [@pace_analysis_1985;@kolbert_ribosomal_2012].
...
tissue damage in mice infected with CDI
...
experience complications due to CDI such as ileus, toxic megacolon, or death in
8-9% of cases [@lucado_clostridium_2012;@kassam_clostridium_2016].
...
experiencing a first or second recurrence [@de_la_villa_real-world_2022].
...
Alternatively, ML can be used pragmatically in fields like medicine,
diagnosis diseases or make predictions about disease outcomes, with the goal of
...
Care must be taken to train, test, validate, and deploy ML models responsibly.
Pitfalls have been identified as pervasive in studies that apply ML; these
...
[@kapoor_leakage_2022;@framework_topcuoglu_2020].
...
data science will empower budding scientists with the skills necessary to
...
In Chapter 2, we present OptiFit, a new OTU clustering algorithm that enables
researchers to fit new data to existing \textit{de novo} OTUs while maintaining
OTU quality.
In Chapter 3, we train ML models to predict the severity of CDI from OTUs
and compare model performance to prior approaches.
In Chapter 4, we introduce two curricula and one software package which help
...
through 4 and discuss future work to build upon this dissertation.
