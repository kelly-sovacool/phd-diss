...
As the different datasets have different proportions of severe cases,
...
In situations where a predicting a severe case may lead clinicians to choose a
treatment option that has an established treatment record, such as oral
...
The most useful metrics to compare - precision

huge increase in auroc may or may not translate to a huge increase in benefit
to patients.
decision threshold analysis instead of just 95th percentile of risk.

Models to guide treatment options. 
The importance of each quadrant of the confusion matrix depends on the
application as well as the nature of the dataset.
While AUROC is easy to interpret, it is not necessarily the best metric to
use when comparing models on imbalanced datasets and for certain clinical applications.
In the case of highly imbalanced data as is the case with CDI severity,
there are very few positive samples, and a model may have a good AUROC but
perform poorly at identifying true positives.
In the case of low-risk and non-invasive treatments such as choosing between
oral antibiotics, a higher number of false positives may be tolerable as long as
treatment cost is not unbearably high.
However, for highly invasive and irreversibly treatments such as colectomy,
