...
Pairing the prior EHR-based model with fidaxomicin would yield an NNB of `r
ehr_fdx_nnb`.
...
#|   - "Full datasets"
#|   - "Intersection of samples with all labels available"
risk_thresh <- read_csv(here('papers','severe-cdi','results','decision_thresholds.csv'))
risk_thresh %>%
  filter(Dataset == 'Full') %>%
  select(-Dataset) %>%
  kable() %>%
...
thresh_dat %>% filter(strategy == 'model') %>% group_by(outcome, dataset) %>% 
  summarize(med_nns = median(nns),
            mean_nns = mean(nns),
            med_diff = median(diff_95th_pct),
            mean_diff = mean(diff_95th_pct))
med_nns <- thresh_dat %>% 
    filter(strategy == 'model', outcome != 'idsa', !(outcome == 'pragmatic' & dataset == 'int')) %>%
    group_by(outcome, dataset) %>% 
    slice_min(diff_95th_pct) %>% filter(nns < Inf) %>% summarize(med_nns = median(nns))
thresh_dat %>% 
    filter(strategy == 'model', outcome != 'idsa', !(outcome == 'pragmatic' & dataset == 'int')) %>%
    group_by(outcome, dataset) %>% 
    slice_min(diff_95th_pct) %>% right_join(med_nns %>% rename(nns = med_nns)) %>% 
  select(outcome, dataset, nns, diff_95th_pct, decision_threshold) %>% unique()
```
