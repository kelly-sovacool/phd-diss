# Predicting Severity of _C. difficile_ Infections from the Taxonomic Composition of the Gut Microbiome
...
library(schtools)
library(tidyverse)
load(here('papers','severe-cdi',"results", "stats.RData"))
table_font_size <- 10
...
`r sovatools::insert_image_md('papers/severe-cdi/paper/figures/flowchart_sankey.png')`
\justify
\footnotesize
...
read_csv(here('papers','severe-cdi','results','count_table_full.csv')) %>%
  format_table()
read_csv(here('papers','severe-cdi','results','count_table_int.csv')) %>%
  format_table()
```
...
```{r model_comps_full}
#| output: false
model_comps <- read_csv(here('papers','severe-cdi','results','model_comparisons.csv'))
...
`r sovatools::insert_image_md('papers/severe-cdi/paper/figures/ml-performance.png')`
\justify
\footnotesize
...
`r sovatools::insert_image_md('papers/severe-cdi/paper/figures/feature-importance.png')`
\justify
\footnotesize
...
positive and is calculated as the reciprocal of precision (@eq-nns) [@rembold_number_1998].
...
benefit from the treatment and is calculated as the reciprocal of the absolute
risk reduction (ARR) from randomized controlled trials (@eq-arr and @eq-nnt)
[@cook_number_1995;@irwig_smart_2008].
...
We focus on the performance at the 95th percentile of risk in order to compare
to prior studies based on EHRs that reported model precision at this risk
threshold.
...
`r pragmatic_nns` for the pragmatic definition (@fig-risk).
...
true, at best `r min_fdx_nnb` and at worst `r max_fdx_nnb` patients would need to be
...
smaller set of manually curated variables, achieving precision values of `r ehr_prec` 
(NNS = `r ehr_nns`) for the EHR model and `r curated_prec` (NNS = `r curated_nns`)
for the curated model at the 95th percentile of risk [@li_using_2019;@rao_clostridium_2015].
Pairing the prior EHR-based model with fidaxomicin would yield an NNB of `r ehr_fdx_nnb`.
Thus the best OTU-based model did not outperform the EHR-based model,
...
::: {#fig-risk fig-cap="Model performance in terms of the number needed to screen across risk thresholds."}
```{r plot_risk_thresholds}
thresh_dat <- read_csv(here('papers','severe-cdi', 'results',
                            'thresholds_results_aggregated.csv')) %>% 
  mutate(predicted_pos_frac = (tp + fp) / (tp + fp + tn + fn),
         diff_95th_pct = abs(predicted_pos_frac - 0.05),
         nns2 = 1/Precision)
nns_95th_pct <- read_csv(here('papers','severe-cdi','results','decision_thresholds.csv'))
thresh_dat %>% 
  filter(strategy == 'model', outcome != 'idsa', dataset=='full') %>% 
  mutate(predicted_pos_frac = round(predicted_pos_frac,2)) %>% 
  group_by(predicted_pos_frac, outcome, dataset) %>% 
  summarise(
  ...
  #            alpha = 0.08) +
  geom_line(aes(x = predicted_pos_frac, y = median_nns, color = outcome),
            alpha = 0.6) +
  ...
  guides(fill = 'none') +
  labs(y = "Median NNS", x = 'Risk Threshold') +
  theme_sovacool() +
...
thresh_dat %>% 
  ...
  mutate(predicted_pos_frac = round(predicted_pos_frac,2)) %>% 
  ggplot() +
  stat_summary(aes(x = predicted_pos_frac, y = nns, color = outcome),
...
\footnotesize
TODO caption.
The confusion matrix was computed for the decision threshold at the 95th percentile of risk for each severity prediction model, which corresponds to 5% of cases predicted to have a severe outcome. The number needed to screen (NNS) to identify one true positive is the reciprocal of precision.
:::
...
`r sovatools::insert_image_md('papers/severe-cdi/paper/figures/prc_curves.png')`
\justify
\footnotesize
...
`r sovatools::insert_image_md('papers/severe-cdi/paper/figures/cdiff-otu.png')`
\justify
\footnotesize
